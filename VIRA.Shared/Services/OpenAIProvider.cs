...
public class OpenAIProvider : IProviderService
{
    ...
    public OpenAIProvider(HttpClient httpClient, ISecureStorageManager storageManager)
    {
    ...
    {
        var apiKey = await _storageManager.GetApiKeyAsync("OpenAI");
    ...
    {
        _apiKey = apiKey;
        await _storageManager.SaveApiKeyAsync("OpenAI", apiKey);
    }
    ...
    public async Task<ChatMessage> SendMessageAsync(string message, List<ChatMessage> history)
    {
        if (string.IsNullOrEmpty(_apiKey))
        {
            throw new InvalidOperationException("OpenAI API Key has not been configured. Please set your API key in Settings.");
        }
        ...
        {
            throw new Exception($"Failed to send message to OpenAI: {ex.Message}", ex);
        }
    }

    /// <summary>
    /// Sends a message to OpenAI API and returns a streaming response
    /// </summary>
    public async Task<Stream> SendMessageStreamAsync(string message, List<ChatMessage> history)
    {
        ...
        {
            throw new Exception($"Failed to stream message from OpenAI: {ex.Message}", ex);
        }
    }

    /// <summary>
    /// Gets the list of available models for OpenAI
    /// </summary>
