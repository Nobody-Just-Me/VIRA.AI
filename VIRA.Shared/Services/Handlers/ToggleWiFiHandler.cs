...
/// <summary>
/// Handler for toggling WiFi on/off
/// Pattern: "nyalakan|matikan wifi"
/// </summary>
public class ToggleWiFiHandler : ICommandHandler
{
    public async Task<CommandResult> HandleAsync(Match match, ConversationContext context)
    {
        ...
        string response = $"{emoji} {char.ToUpper(actionText[0]) + actionText.Substring(1)} WiFi... (Fitur ini akan diimplementasikan di task 6.4)";

        return await Task.FromResult(new CommandResult(
            response: response,
            action: new { Type = "toggle_wifi", Enable = enable },
            confidence: 1.0f,
            speak: true
        ));
    }
