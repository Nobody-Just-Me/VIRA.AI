...
/// <summary>
/// Handler for toggling Bluetooth on/off
/// Pattern: "nyalakan|matikan bluetooth"
/// </summary>
public class ToggleBluetoothHandler : ICommandHandler
{
    public async Task<CommandResult> HandleAsync(Match match, ConversationContext context)
    {
        ...
        string response = $"{emoji} {char.ToUpper(actionText[0]) + actionText.Substring(1)} Bluetooth... (Fitur ini akan diimplementasikan di task 6.4)";

        return await Task.FromResult(new CommandResult(
            response: response,
            action: new { Type = "toggle_bluetooth", Enable = enable },
            confidence: 1.0f,
            speak: true
        ));
    }
