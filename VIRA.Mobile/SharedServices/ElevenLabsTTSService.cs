...
public class ElevenLabsTTSService : ITTSService
{
    ...
    private string _voiceId = "21m00Tcm4TlvDq8ikWAM"; // Default: Rachel (Female, calm)
    
    // Voice ID dari ElevenLabs Pre-made Voices (Free Tier Compatible)
    // Default: Rachel - Female, calm, natural voice (FREE)
    // User can change this via Settings
    ...
    public void SetVoiceId(string voiceId)
    {
        if (!string.IsNullOrEmpty(voiceId))
        {
            _voiceId = voiceId;
            #if __ANDROID__
            Android.Util.Log.Info("VIRA_ElevenLabs", $"Voice ID set: {voiceId}");
            #endif
        }
    }
    ...
    public async Task<byte[]> SynthesizeSpeechAsync(string text)
    {
        ...
        try
        {
            ...
            var url = $"{BaseUrl}/text-to-speech/{_voiceId}";
            #if __ANDROID__
            Android.Util.Log.Info("VIRA_ElevenLabs", $"📡 Sending request to: {url}");
            Android.Util.Log.Info("VIRA_ElevenLabs", $"🎤 Using Voice ID: {_voiceId}");
            #endif
